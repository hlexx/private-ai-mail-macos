...
| AI summary | Supported narrowly for fetched Gmail attachments with extractable local text, HTML, JSON, CSV, or text-bearing PDFs. Summaries must include verifiable evidence from extracted chunks; unsupported and evidence-missing states are not returned as successful summaries. | Not wired end-to-end yet because Outlook attachment bytes are not connected to AttachmentRAG in app composition. | `AttachmentSummaryOrchestrator`, `AttachmentSummaryStore`, `AIAttachmentSummary`, `AttachmentRAGTests`, `ThreadFeatureTests`. |
